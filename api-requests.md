...
| GET    | `/api/admin/lemmas/search`         | Поиск лемм для аннотации токена                  |
| POST   | `/api/admin/tokens/:id/annotate`   | Привязать токен к правильной лемме               |
...
### 7.8 GET `/api/admin/lemmas/search` (CAN_EDIT_TEXTS)

Поиск лемм по базовой форме или нормализованной форме. Используется в диалоге привязки токена.

**Query-параметры:**

| Параметр   | Тип    | По умолчанию | Описание                      |
| ---------- | ------ | ------------ | ----------------------------- |
| `q`        | string | —            | Строка поиска (от 2 символов) |
| `language` | string | `CHE`        | Язык: CHE, RU, EN, AR        |
| `limit`    | number | `20`         | Максимальное число результатов (max 50) |
...
### 7.9 POST `/api/admin/tokens/:id/annotate` (CAN_EDIT_TEXTS)

Привязывает токен к правильной лемме. Инвалидирует кэш токена.
...
| Scope    | Описание                                                                                     |
| -------- | -------------------------------------------------------------------------------------------- |
| `local`  | Создаёт/обновляет `TokenAnalysis` с `isPrimary=true` только для этого токена (для омографов) |
| `global` | Дополнительно создаёт `MorphForm` для нормализованной формы токена — глобально для всех текстов |
